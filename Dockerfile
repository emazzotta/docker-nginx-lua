...
ENV GOOGLE_PAGESPEED_PSOL_VERSION=1.12.34.2-x64
...
RUN wget --no-check-certificate http://nginx.org/download/nginx-$NGINX_VERSION.tar.gz -P $NGINX_TEMP_DIR/ && \
    ...
    wget --no-check-certificate https://dl.google.com/dl/page-speed/psol/$GOOGLE_PAGESPEED_PSOL_VERSION.tar.gz -O psol-$GOOGLE_PAGESPEED_PSOL_VERSION.tar.gz && \
